                           ...
                           <center>
                            ...
                            @if(isset($product_pro_post->productPostes) && count($product_pro_post->productPostes) > 0)
                              ...
                              @if($current_post->posting == 1)
                                ...
                                <a href="javascript:;" data-toggle="modal" data-toggle="tooltip" data-placement="top" title="{{($current_post->posting == 0) ? 'Stoped' : 'Running' }}" class="{{($current_post->posting == 0) ? 'cls-stop' : '' }} {{$group_color_class}} " id="schedule_row_{{$current_post->id}}" onclick="scheduleProduct('{{@$campaign_current->id}}','{{$key}}','{{$id}}','{{$product_pro_post->id}}','{{$product_pro_post->type->name}}','{{$product_pro_post->category}}','{{$product_pro_post->promotion_product_type_id}}', '{{$current_post->group_code}}','{{$current_post->group_id}}','{{$current_post->id}}','{{$socials}}', '{{$current_post->date}}','{{$current_post->last_date}}','{{$store}}','{{$current_post->range}}', 'current', '{{$group_id}}','{{$selected_cate}}', '{{($product_pro_post->subCategory) ? $product_pro_post->subCategory->id : null}}')">
                          ...
                          <td class="argn-popup-td next-td next-section" style="vertical-align: middle; display:none;"><center>
                            ...
                            @if(isset($product_pro_post->productPostes) && count($product_pro_post->productPostes) > 0)
                              ...
                              <div>
                                ...
                                <center>
                                @if($product_pro_post->is_deleted == 0)
                                <a href="javascript:;" title="{{($next_post->posting == 0) ? 'Stoped' : 'Running' }}" class="{{($next_post->posting == 0) ? 'cls-stop' : '' }} {{$group_color_class}} " id="schedule_row_{{$next_post->id}}" onclick="scheduleProduct('{{@$campaign_next->id}}','{{$key}}','{{$id}}','{{$product_pro_post->id}}','{{$product_pro_post->type->name}}','{{$product_pro_post->category}}','{{$product_pro_post->promotion_product_type_id}}', '{{$next_post->group_code}}','{{$next_post->group_id}}','{{$next_post->id}}','{{$socials}}', '{{$next_post->date}}','{{$next_post->last_date}}','{{$store}}','{{$next_post->range}}', 'next', '{{$group_id}}','{{$selected_cate}}', '{{($product_pro_post->subCategory) ? $product_pro_post->subCategory->id : null}}')">
