            ...
            <div class="sub-setting-loop col-md-12" >
                      <form name="change_schedule" class="change_schedule" class="" action="{{url('add/group')}}" method="post">
                          ...
                          <input type="hidden" name="old_group_code" id="group_code" value="{{$group_code}}">
                            ...
                            @if($group_type == 9 || $group_type == 6 || $type == 'Promo Video' || $type == 'Category Video')
                            <div class="col-md-{{$cate_col}}">
                                @foreach($cate as $k => $c)
                                    <label for="{{$c}}">{{$c}} {{ ($k < count($cate)-1) ? ',' : ''}} </label> 
                                @endforeach
                            </div>
                            @else
                            <div class="col-md-{{$cate_col}}">
                            ...
                            @if($post_type == 1)
                            @if($group_type == 9 || $group_type == 6 || $type == 'Promo Video' || $type == 'Category Video')
                            <input type="hidden" name="organic_multiple" id="organic_multiple" value="1">
                             <select class="form-control selectpicker{{@$setting->id}} multiselect" name="schedule_group[]" onchange="loadDetails(this.value, 1)" multiple>
                                      @foreach($collection as $group)
                                        {{-- @if( count($group->productsQuantity) > 0 ){ --}}
                            ...
                            <button type="button" onclick="saveNewSchedule()" class="btn btn-info save-btn float-right">Save</button>
                          </div>
                    </form>
...
$('.selectpicker').css('display', 'block');
var m_groups = <?php echo json_encode(@$selected_groups) ?>;
console.log(m_groups);
$('.selectpicker').selectpicker('val', m_groups);
});
...
function loadDetails(value, action = null) {
  console.log($( ".multiselect option:selected" ).val());
  if(action) {
    value = $(".multiselect option:selected").last().val();  
  }
  console.log(value);
    $('.body-data').html('');
  if(value) {
...
function saveNewSchedule() {
  ...
  $.ajax({
    url: "{{url('/productgroup/svae/change/schedule')}}",
    type: "POST",
