                      ...
                      <form name="change_schedule" class="change_schedule" class="" action="{{url('add/group')}}" method="post">
                          ...
                          <input type="hidden" name="old_group_code" id="group_code" value="{{$group_code}}">
                            <div class="row" id="">
                              ...
                              @php $cate_col = 6; $group_col = 4; @endphp
                              @endif
                            <div class="col-md-2">
                            ...
                            <div class="col-md-{{$cate_col}}">
                                ...
                                @foreach($cate as $k => $c)
                                <div class="col-md-{{(count($cate) < 2) ? '6' : '4'}} col-grid">
                                   <input type="radio" id="{{$c}}" id="catess" name="selected_category" value="{{$c}}" onchange="getgroupOfCategory('{{$group_type}}','{{$type}}','{{$c}}','{{$sub_category}}')" {{($group_name == $c) ? 'checked' : ''}}>
                                    <label for="{{$c}}" {{$k == 0 ? 'checked' : ''}}>{{$c}} </label>  {{(count($cate) < 2) ? "(".$subCat_name.")" : ''}}
                                </div>
                              @endforeach
                            </div>
                            ...
                            <button type="button" onclick="saveNewSchedule('{{@$row}}','{{$main_setting}}')" class="btn btn-info save-btn">Save</button>
                          </div>
                          </div>
                    </form>
