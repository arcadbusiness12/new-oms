    ...
    <div class="container-fluid">
      ...
      @endphp
        ...
        <div class="block-header">
            <div class="col-sm-2">
            <h2>Photographer duty lists</h2>
            </div>
            @forelse($all_models as $key => $model)
              <div class="col-sm-2">
                  @php
                    $request_url = str_replace("model_id=$request_model_id","model_id=$model->user_id",$request_url);
                  @endphp
                <center><a href="{{ $request_url }}" class="{{ $request_model_id == $model->user_id ? 'bottom_border_green' : '' }}"><strong>{{ $model->firstname }} {{ $model->lastname }}</strong></a></center>
              </div>
            @empty
                        ...
                        <div class="panel-body">
                          ...
                          <div class="col-12 month-row">
                            <div class="col-2 form-div">
                              <form method="get" action="{{route('employee-performance.photography.product-shoot')}}">
                                <input type="hidden" name="previous_month" value="{{$previousMonth}}">
                                <input type="hidden" name="model_id" class="model_id" value="{{$request_model_id}}">
                            ...
                            <div class="col-2 form-div {{ ($k > 1) ? 'hidden-xs' : '' }}">
                              <form method="get" action="{{route('employee-performance.photography.product-shoot')}}">
                                      ...
                                      <tbody>
                                          ...
                                          <tr id="" style="border-top: 1px solid gray !important;">
                                            ...
                                            @php
                                                  ...
                                                  @endphp
                                                    @if(@$new_arrivals_data[$day['hiddn_date']][$i])
                                                      @php
                                                       ...
                                                       $posted = 0;
                                                       $diabale_action = "";
                                                       $id = $new_arrivals_data[$day['hiddn_date']][$i]['id'];
                                                       ...
                                                       $posting_details = $new_arrivals_data[$day['hiddn_date']][$i]['photographyPosting'];
                                                       $products        = $new_arrivals_data[$day['hiddn_date']][$i]['products'];
                                                    ...
                                                    <div class="col-sm-12 btn-designed-icon{{$scheduled_id}}">
                                                      @if( $shoot == 0 )
                                                          @if( session('user_group_id') == 1 )
                                                            <a href="javascript:void()" data-toggle="modal" data-target="#new_group_entry_model" onclick="editGroup('{{ $day['hiddn_date'] }}',{{ $id  }})"><i class="material-icons">edit</i></a>
                                                          @endif
                                                          @if( session('user_group_id') == 1 || isset($user_perms['employee-performance/photography/save-shoot-data']) )
                                                      ...
                                                      @else
                                                          @forelse($social_channel as $key => $val)
                                                            ...
                                                            if( $val->id == 1 ){
                                                              ...
                                                              $button_color = "#FF0000";
                                                              $popup_modal_attributes = 'data-toggle=modal data-target=#video_popup';
                                                            }
                                                            if( count($posting_details) > 0 ){
                                                              //echo $group_name;
                                                            ...
                                                            @endphp
                                                              ...
                                                              @else
                                                                @if( (session('user_group_id') == 1 || isset($user_perms['employee-performance/photography/save-shoot-posting'])) && count($products) )
                                                                  <a href="javascript:;" onclick="postProducts('{{$scheduled_id}}','{{$group_name}}',{{ $task_id }},{{ $val->id }})" style="background:{{ $button_color }} !important" class="btn btn-xs btn-info btn-designed btn-designed{{$scheduled_id.$val->id}}" {{ $popup_modal_attributes }} {!! $popup_modal_attributes !!}>{!! $material_icon !!} {{ $val->name  }} </a>
                                                                @else
                                                                  <span class="label label-danger">Posting permission denied<br>or products disabled.</span>
                                                                  @break;
                                                                @endif
                                                              @endif
                                                          @empty
                                                  ...
                                                  @else
                                                    @if( session('user_group_id') == 1 ||  session('user_group_id') == 21 )
                                                      <a href="javascript:void()" data-toggle="modal" data-target="#new_group_entry_model" onclick="postingDate('{{ $day['hiddn_date'] }}')"><strong>+</strong></a>
                                                    @else
                                                      -
                                                    @endif
                                                  @endif
...
@include('employee_performance.photography.popup_new_group');
@include('employee_performance.photography.popup_video_links');
@endsection
@push('scripts')
...
<script>
  function postingDate(post_date){
    $('#posting_date').val(post_date);
    $('#row_id').val('');
  }
  function editGroup(post_date,id){
    $('#posting_date').val(post_date);
    $('#row_id').val(id);
  }
...
function postProducts(product_group_id,product_group_name,task_id,social_id){ 
  ...
  $.ajax({
    url: "{{ route('employee-performance.photography.saveShootPosting') }}",
    type: "POST",
...
function shootProduct(product_group_id,product_group_name,task_id){
  var id = task_id;
  $.ajax({
    url: "{{ route('employee-performance.photography.saveShootData') }}",
    type: "POST",
