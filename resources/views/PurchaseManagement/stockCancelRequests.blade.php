                                ...
                                <div class="card-header white">
                                    <div class="panel panel-default">
                                        <div class="panel-body">
                                            <form name="filter_orders" id="filter_orders" method="get" action="<?php echo route('shipped.stock.cancelled.requests') ?>">
                                                {{ csrf_field() }}
                                                <div class="row">
                                                    <div class="col-sm-4">
                           ...
                           <?php if($orders['data']) { ?>
                            ...
                            <div class="card order_list mb-4">
                                <div class="row top_row">
                                    <div class="col-xs-4 col-sm-4 text-black mb-4 mt-2"><b>Order Number: #<?php echo $order['shiped_order_id'] ?></b></div>
                                    ...
                                    <div class="col-xs-4 col-sm-4 text-right">
                                        <?php if($order['shipped_order']['urgent']) { ?>
                                            <div class="badge badge-warning orange darken-1"  style="font-size: 15px;"><b>Urgent</b></div>
                                        <?php } ?>
                                        <?php if(isset($order['shipped_order']['ship_by_sea']) && $order['shipped_order']['ship_by_sea']) { ?>
                                            <div class="badge badge-warning orange darken-1">Ship By Sea</div>
                                        <?php } ?>
                                    </div>
                                </div>
                                <?php foreach ($order['shipped_order']['order_products'] as $product) { ?>
                                <div class="product_list_row">
                                    ...
                                    <div id="product-option<?php echo $order['order_id'] . $product['product_id'] ?>" class="options_row table-responsive collapse">
                                        <table class="table">
                                            ...
                                            <?php $i = 0; foreach ($product['products_sizes'] as $option) { 
                                                if(count($option) <= 0) {
                                                    continue;
                                                }
                                                $i++; ?>
                                            <tr class="single_option_row">
                                                <td class="col-xs-2">
                                                    ...
                                                    <?php } ?>
                                                    <div><input type="text" class="form-control" value="<?php echo @$option[0]['name'] ?> - <?php echo @$option[0]['value'] ?>" readonly></div>
                                                </td>
                                                <td class="col-xs-2">
                                                    <?php if($i == 1) { ?>
                                                    <label class="control-label">Order Quantity</label>
                                                    <?php } ?>
                                                    <div><input type="text" class="form-control" value="<?php echo @$option[0]['order_quantity'] ?>" readonly></div>
                                                </td>
                                                <td class="col-xs-2">
                                                    <?php if($i == 1) { ?>
                                                    <label class="control-label">Price</label>
                                                    <?php } ?>
                                                    <div><input type="text" class="form-control price" value="<?php echo @$option[0]['price'] ?>" readonly/></div>
                                                </td>
                                            </tr>
                                            <?php } ?>
                                            <?php }else{ ?>
                                ...
                                <div class="row mb-4">
                                    ...
                                    <form action="<?php echo route('update.stock.cancel.order.request') ?>" method="post">
                                        {{ csrf_field() }}
                                        <input type="hidden" name="order_id" value="<?php echo $order['order_id'] ?>">
                                        <input type="hidden" name="shiped_order_id" value="<?php echo $order['shiped_order_id'] ?>" />
                                        <input type="hidden" name="supplier" value="<?php echo ($order['order_supplier']) ? $order['order_supplier']['user_id'] : '' ?>">
                                        <div class="col-xs-4 col-sm-4 col-grid ">
                                          <button type="submit" name="submit" value="update_stock_cancelled" onclick="return confirm('Are You Sure Want To Approve ?')" class="btn btn-success active form-control">Approve</button>
                                        </div>
                                        <div class="col-xs-4 col-sm-4 col-grid">
                                           <button type="submit" name="submit" value="cancel_stock_cancelled" onclick="return confirm('Are You Sure Want To Cancel ?')" class="btn btn-danger active form-control">Cancel</button>
                                        </div>
                                    </form>
                                    </div>
