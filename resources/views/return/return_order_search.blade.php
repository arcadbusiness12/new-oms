    ...
    <div class="card">
        <div class="panel panel-default">
            <div class="panel-body table-responsive">
                @if($order)
                <form action="{{ route('return.update') }}" method="post" name="update_quantity" id="form_update_quantity">
                    ...
                    <table class="table">
                        ...
                        <tr class="order_list">
                            <td class="col-xs-3 text-center">{{  $order->order_id }}-2</td>
                            {{--  <td class="col-xs-3 text-center"><?php echo $order['total'] ?></td>  --}}
                            <td class="col-xs-3 text-center">{{ $order->created_at }}</td>
                            <td class="col-xs-3 text-center"><a href="{{ URL::to('/orders/print/label/' . $order->order_id) }}" target="_blank"><button type="button" class="btn btn-primary">Print</button></a></td>
                        </tr>
                        <?php $returned = 0;
                        ...
                        <tr class="product_list">
                            ...
                            <td class="col-xs-4 text-center">{{ $product->sku }} <br>
                                <label  class="float-right"><input type="checkbox" class="float-right" name="isdemage[{{ $product->productOption->option_value_id }}]" > Is Demage</label>
